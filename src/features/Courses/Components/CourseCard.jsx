    ...
    <div className="bg-white dark:bg-[#292D4A] rounded-md overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
      ...
      <div className="p-5 flex flex-col gap-4">
        <div className="flex justify-between items-start">
          <div>
            <h4 className="text-header-text font-bold text-lg mb-1">{course.title}</h4>
            <p className="text-content-text text-xs flex items-center gap-2">
              {course.instructor} • 
              <span className="flex items-center gap-1">
                {course.rating} <Star size={12} className="fill-secondary text-secondary" />
              </span>
            </p>
          </div>
          <span className="text-header-text font-bold text-lg">${course.price}</span>
        </div>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button className="bg-primary hover:bg-primary/90 text-white px-6 py-2 rounded-lg text-sm font-bold transition-all">
            View all
          </button>
        </div>
      </div>
