    ...
    >
      ...
      <div
        className="px-4 py-2 text-xs font-semibold uppercase tracking-wider
                      border-b border-gray-100 dark:border-white/10 mb-1"
        ...
        <div key={child.id} className="relative">
          {child.children?.length ? (
            <>
              <button
                onClick={() =>
                  setOpenChild(openChild === child.id ? null : child.id)
                }
...
function NavItem({ item, depth = 0, collapsed }) {
  ...
  // ── Leaf item ─────────────────────────────────────────
  if (!hasChildren) {
    // SUB-ITEM (depth > 0): no bg on hover, animated line + text slide
    if (depth > 0) {
      return (
        <li>
          <NavLink
          ...
          >
            ...
            />
            {/* Text with subtle left-slide */}
            <span
            className="hover:text-sidebar-icon"
              style={{
    ...
    // TOP-LEVEL LEAF: full primary bg on hover/active
    return (
      <li>
        <NavLink
          to={item.path || "#"}
  ...
  // ── Parent item ───────────────────────────────────────
  // SUB-PARENT (depth > 0): no bg, just text color change
  if (depth > 0) {
    return (
      <li>
        <button
        ...
        >
          ...
          />
          <span
            className="flex-1 text-left truncate text-sidebar-icon"
            style={{
  ...
  // TOP-LEVEL PARENT: full primary bg on hover/open/active
  return (
    <li>
      <button
        onClick={() => setOpen(!open)}
...
function FullSidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div
        className={`flex items-center border-b border-gray-100 dark:border-white/5
