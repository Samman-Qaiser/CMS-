...
function FlyoutMenu({ item, anchorRef }) {
  ...
  return createPortal(
    ...
    >
      {/* Header label */}
      <div
        className="px-4 py-2 text-xs font-semibold text-sidebar-text/60
    ...
    return (
      <li>
        ...
        >
          ...
          {depth === 0 && Icon && (
            <Icon className="w-5 h-5 shrink-0 text-sidebar-text" />
          )}
  ...
  return (
    <li>
      ...
      >
        {depth === 0 && Icon && (
          <Icon
            className={`w-5 h-5 shrink-0 ${open ? "text-primary" : "text-primary"}`}
          />
        )}
        {depth > 0 && (
          <span
            className={`block h-px w-3 ${open ? "bg-white" : "bg-primary"}`}
          />
        )}
